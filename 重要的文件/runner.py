...
class Runner:
    ...
    def _component(self, name: str) -> Type:
        """Get skrl component (e.g.: agent, trainer, etc..) from string identifier 输入str是类型名字

        :return: skrl component
        \n python Type是一个类型提示，表示返回值是一个类型（类）。
        """
        component = None
        name = name.lower()
        # model
        ...
        # agent  统一入口点设计。灵活的配置：在YAML中指定"ppo"或"ppo_default_config"，牺牲了一点类型安全性，换来了极大的配置灵活性
        ...
        for agent_id in possible_agents:
            ...
            if separate:
                for role in models_cfg:
                    ...
                    model_class = self._component(model_class)    # get model class
                    ...
                    models[agent_id][role] = model_class(
                        ...
                        device=device,
                        **self._process_cfg(models_cfg[role]),
                    )  # model_class = self._component(model_class) 在上面第235行被调用
            # shared models
            else:
                ...
                for role in roles:
                    ...
                    if not model_structure:
                        raise ValueError(f"No 'class' field defined in 'models:{role}' cfg")
                    del models_cfg[role]["class"]  # 删除class字段以避免后续处理时被误认为是模型参数
                    structure.append(model_structure)
                    parameters.append(self._process_cfg(models_cfg[role]))
                model_class = self._component("Shared")  # 返回shared_model类
                # print model source
                source = model_class(
                    observation_space=observation_spaces[agent_id],
                    action_space=action_spaces[agent_id],
