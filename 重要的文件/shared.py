...
def shared_model(
    ...
    models = [{"class": item} for item in structure]  # 一个list
    ...
    for i, model in enumerate(models):
        ...
        )  # 生成容器和输出 这两个都是字符串，里面定义了神将网络的结构

    # network definitions   networks_common 是两个模型都需要的网络定义，forward_common 是两个模型都需要的前向传播步骤.这两个都是字符串
    networks_common = []
    forward_common = []
    ...
    # build template 这里定义了SharedModel的类定义
    ...
    def compute(self, inputs, role=""):
        ...
        if role == "{roles[0]}":
            ...
            return {get_return(structure[1])}
    """
    # return source 这里返回的是一个字符串，包含了SharedModel的定义
    if return_source:
        return template

    # instantiate model
    _locals = {}
    exec(template, globals(), _locals)  # exec函数会执行template字符串中的代码，并将定义的SharedModel类放在_locals字典中
